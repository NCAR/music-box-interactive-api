        ...
        if 'subtype' in self.unitDict[initial_unit]:
            if self.unitDict[initial_unit]['subtype'] == self.unitDict[final_unit]['subtype']:
                ...
                def unitChange(value, kwargs={}):
                    ...
                    scale_to_base_unit = scaleToBase(initial_unit, self.unitDict)
                    ...
                    scale_to_final_unit = scaleFromBase(final_unit, self.unitDict)
                    ...
                    for key in kwargs:
                        if 'units' not in key:
                            ...
                            if unitName in kwargs:
                                ...
                                scaler_to_base = scaleToBase(inputUnit, self.unitDict)
                                kwValue = scaler_to_base(kwValue)
                            converted_kwargs.update({key.replace("_", ' '): kwValue})
                    
                    value = scale_to_base_unit(value)
                    value = convert_to_base_type(value, {j: converted_kwargs[j] for j in convert_to_base_type_args})
                    value = convert_to_final_type(value, {k: converted_kwargs[k] for k in convert_to_final_type_args})

...
def make_additional_argument_form(arguments):
    ...
    for arg in arguments:
        arg_unit_options = [key for key in unitDict if unitDict[key]['type'] == arg]
        response.write('<div class="input-group mb-3"><span class="input-group-text">')
        response.write(arg)
        response.write('</span><select class="btn btn-primary dropdown-toggle" id="arg_')
        response.write(arg)
        response.write('">')

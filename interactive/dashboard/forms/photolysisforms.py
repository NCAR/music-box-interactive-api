...
class PhotoForm(forms.Form):    
    def __init__(self, *args, **kwargs):
        ...
        for key in inits['reactions']:
            self.fields[key + '.r_form'] = forms.CharField(initial=inits['reactions'][key], widget=forms.TextInput(attrs={'savebutton': 'photoSave', 'class': 'form-control'}))
            self.fields[key + '.init'] = forms.FloatField(initial=inits['initial value'][key], widget=forms.TextInput(attrs={'savebutton': 'photoSave', 'class': 'form-control'}))

...
def isBlocked(blocked, element_or_reaction):
    for bl in blocked:
        if bl in element_or_reaction and len(blocked) is not 0:
            return True
    return False
...
def find_edges_and_nodes(contained_reactions, reactions_names_dict, reactions_json, widths_dict, blocked):
        ...
        try:
            ...
            if 'reactants' in reaction:
                ...
                for r in reactants:
                    ...
                    # print("checking if reaction is blocked:", reaction_name)
                    # check if in blocked list
                    if isBlocked(blocked, reaction_name) == False:
                        edges.update({edge: {}})
                        edge_colors.append("#cfe0fd")
                    else:
    ...
    return {'edges': list(edges), 'species_nodes': list(s_nodes), 'reaction_nodes': r_nodes, 'edge_colors': edge_colors}

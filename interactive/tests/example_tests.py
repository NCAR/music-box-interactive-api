...
# The results from client testing and the controller function are different, so rounding is needed
...
def test_chapman_example_api(api_client):
    ...
    rounded_json_data = round_floats(convert_keys_to_strings(json_data), precision=5)
    rounded_expected_data = round_floats(convert_keys_to_strings(expected_data), precision=5)
